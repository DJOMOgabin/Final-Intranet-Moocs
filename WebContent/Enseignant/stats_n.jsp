...
<%@ page import = "java.sql.Connection, java.sql.ResultSet, java.sql.Statement, java.util.ArrayList, Modele.Cours, Modele.constante,Modele.CopieEtudiant, Modele.User, Hibernate.HibernateCopie" %>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
>>>>>>> 647841db7d091c842900a7c18b2079aeb8d6ee5b
			                ...
			                <li class="list-group-item">
			                  <b>Epreuve</b> <a class="pull-right"><%=request.getParameter("epreuve") %></a>
			                </li>
			                <%ArrayList<CopieEtudiant> liste = HibernateCopie.getetudiant(request.getParameter("title"), request.getParameter("epreuve"));  %>
			                <li class="list-group-item">
			                  <b># Etudiants</b> <a class="pull-right"><%=liste.size() %></a>
			                </li>
			                <% double nbTot = teacher.getNombreEtdIns(courMoocs.getId(request.getParameter("title"))); %>
			                
			                <li class="list-group-item">
			                  <b>% Participation</b> <a class="pull-right"><%=String.format("%.2f",(liste.size()/nbTot)*100)%> %</a>
			                </li>
			            </ul>
...
<script src="../js/ace.js"></script>
   ...
   $('#table3x').DataTable({
    ...
    $('#imprime').click(function(e){
       ...
       doc.text('UNIVERSITE DE YAOUNDE I',77,40);
       doc.text('UNIVERSITY OF YAOUNDE I',412,40);
       doc.text('*******',110,55);
       doc.text('*******',450,55);
       doc.text('ECOLE NATIONALE SUPERIEURE POLYTECHNIQUE',35,70);
       doc.text('NATIONAL ADVANCED SCHOOL POLYTECHNIQUE',370,70);
       doc.text('*******',110,85);
       doc.text('*******',450,85);
       doc.text('DEPARTEMENT DE GENIE INFORMATIQUE',45,95);
       doc.text('DEPARTMENT OF COMPUTER ENGINEERING',380,95);
       
       
       doc.setFontSize(15);
       doc.text('Titre du Cours : <%=request.getParameter("title") %>', 62,130);
        doc.text('Titre de L\'Epreuve : <%=request.getParameter("epreuve") %>', 62,160);

        doc.setFontSize(20);
        doc.text('Liste des etudiants/notes',60,doc.autoTableEndPosY() + 200);
        var val = doc.autoTableEndPosY()+200;
